...
export function QuickAddEConversationsDialog({ baseUrl, onAdd, onClose }: Props) {
  ...
  function handleKeyDown(e: React.KeyboardEvent) {
    ...
    if (e.key === 'Enter' && selected.length > 0) onAdd(selected)
  }
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-700 shrink-0">
          <span className="text-sm font-semibold text-gray-200">Quick Add E-Conversations</span>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-700 shrink-0">
          <span className="text-xs text-gray-500">
            Each selected asset creates one e-conversation (background only, role: other, content: …).
          </span>
          <div className="flex gap-2">
            ...
            >
              Add {selected.length > 0 ? `${selected.length} ` : ''}Conversation{selected.length !== 1 ? 's' : ''}
            </button>
          </div>
        </div>
      </div>
